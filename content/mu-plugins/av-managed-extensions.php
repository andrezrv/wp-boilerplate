...
\add_action( 'muplugins_loaded', function () {
	 ...
	 */
	$feature->set_callback( 'views_plugins', function ( array $views ) use ( $is_managed_plugin ): array {
		$count   = count( array_filter( array_keys( get_plugins() ), $is_managed_plugin ) );
		$current = isset( $_GET['plugin_status'] ) && $_GET['plugin_status'] === 'managed';
	 ...
	 */
	$feature->set_callback( 'all_plugins', function ( array $plugins ) use ( $is_managed_plugin ): array {
		if ( ! isset( $_GET['plugin_status'] ) || $_GET['plugin_status'] !== 'managed' ) {
			return $plugins;
		}
	 ...
	 */
	$feature->set_callback( 'plugin_action_links', function ( array $actions, string $plugin_file ) use ( $is_managed_plugin ): array {
	 ...
	 */
	$feature->set_callback( 'plugin_row_meta', function ( array $meta, string $plugin_file ) use ( $is_managed_plugin ): array {
		if ( $is_managed_plugin( $plugin_file ) ) {
	 ...
	 */
	$feature->set_callback( 'admin_init', function () use ( $is_managed_plugin ): void {
		...
		if ( ! in_array( $action, [ 'delete-selected', 'delete' ], true ) ) {
			return;
		}

		// Single-plugin delete uses ?plugin=; bulk uses ?checked[]=
		$candidates = [];
		if ( isset( $_GET['plugin'] ) ) {
			$candidates[] = sanitize_text_field( wp_unslash( $_GET['plugin'] ) );
		}
		if ( isset( $_GET['checked'] ) && is_array( $_GET['checked'] ) ) {
			foreach ( $_GET['checked'] as $p ) {
	 ...
	 */
	$feature->set_callback( 'admin_notices', function (): void {
		if ( get_current_screen()->id !== 'plugins' ) {
			return;
		}
		if ( empty( $_GET['av_managed_delete_error'] ) ) {
			return;
		}
	 ...
	 */
	$feature->set_callback( 'wp_prepare_themes_for_js', function ( array $themes ) use ( $is_managed_theme ): array {
	 ...
	 */
	$feature->set_callback( 'delete_theme', function ( string $stylesheet ) use ( $is_managed_theme ): void {
		if ( $is_managed_theme( $stylesheet ) ) {
			wp_die(
				esc_html__( 'This theme is managed via Composer and cannot be deleted from the WordPress admin. Remove it from composer.json instead.', 'andrezrv-custom-features' ),
	 ...
	 */
	$feature->set_callback( 'admin_enqueue_scripts', function ( string $hook ): void {
		$css = '
			.av-managed-label {
				color: #646970;
				font-style: italic;
